...
plotFromPrecis <- function (precis, dataset, ylims = c(.5,1.2), list, embedding){
  ...
  results$plotIndividual <- ggplot() + geom_point(data = DFindividual, aes( x = reorder(pw, desc(pw)), y = mean, color = type),
    ...
    labs(title = paste(list, ", ", embedding, ", cosine distances, by protected word", sep = ""))+ 
  ...
  grobIndividual <- ggplotGrob(results$plotIndividual+theme_tufte(base_size = 10) +
                                 theme(legend.position='none',
                                 plot.title.position = "plot",
                                 axis.title.x = element_text(size = 8),
                                 axis.title.y = element_text(size = 8),
                                 plot.title=element_text(size=11)))
  grobOverall <- ggplotGrob(results$plotOverall+theme_tufte(base_size = 10)+
