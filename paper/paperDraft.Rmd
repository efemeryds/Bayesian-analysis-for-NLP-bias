...
ourESplot <- ggplot(dataPLOT )+geom_histogram(aes(x=biasesD0, y = ..density..), bins = 50, alpha = .6)+
  labs(title="Distribution of effect sizes given a null model")+ xlab("effect size") +
  geom_vline(xintercept = ourEffectSize, linewidth = .5,
...
ourStatPlot <- ggplot(dataPLOT )+ geom_histogram(aes(x=svaluesD0, y = ..density..), alpha = .6, bins = 50) +
  labs(title="Distribution of test statistics given a null model")+ xlab("test statistic") +
  geom_vline(xintercept = ourStatistic, linewidth = .5,
             lty = 2, alpha = .5)+theme_tufte(base_size = 13)+
...
\begin{figure}[H]
```{r fig:ourDistancesPlot2e,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "80%", warning = FALSE, message = FALSE}
...
These visualizations can be also handy when it comes to the investigation of the effect that debiasing has on the embedding space. In Figures  \ref{fig:empiricalPriorToDebiasing} and  \ref{fig:empiricalDebiased}
 we  see chosen pair depicting the difference in means with 89% highest posterior density intervals before and after applying debiasing.  
...
### Custom lists used in this paper
