...
It has been suggested\footnote{See for instance {[}1,2,5,6,12,13{]}.}
...
example of this approach is {[}4{]}, where the authors use language
...
the real distribution of occupations {[}7{]}.
...
\emph{Word Embedding Association Test} (\textsf{WEAT}) {[}2{]}, and
...
sample sizes used in the studies are usually small,\footnote{Depending
  on a list for {[}2{]} the range for protected words is between 13 and
...
complaining'' or ``macho''.\footnote{However, for some research-related
  purposes, such as the study of stereotypes across history {[}4{]},
...
instance in {[}6{]}, where it is pointed out that while a higher average
...
\emph{caring}). Further, it is observed in {[}6{]} that most word pairs
...
{[}2{]}. The idea here is that the bias between two sets of target
...
with bias in humans. In {[}2{]} the authors argue that significant
...
methodology is employed in {[}5{]}. The authors employ word embeddings
...
\noindent The \textsf{WEAT} scores reported by {[}2{]} for lists of
...
groups of stereotypical attributes, of the same size.\footnote{16 is the
  ...
  from the other sample sizes in word lists used by {[}2{]}).} Each such
...
Calculate the test statistic and effect size the way {[}2{]} did. Do
...
little to do with the posterior probability distribution.\footnote{Here
  ...
  {[}9{]}. In this study, 120 researchers and 442 students were asked to
...
\item
  {[}3{]} correctly argues that a bias estimate should not be expressed
...
\item
  {[}8{]} introduce the Contextualized Embedding Association Test
...
2016. Semantics derived automatically from language corpora necessarily
contain human biases. \emph{CoRR} abs/1608.07187, (2016). Retrieved from
...
\CSLLeftMargin{{[}11{]} }%
\CSLRightInline{John Kruschke. 2015. \emph{Doing {B}ayesian data
analysis (second edition)}. Academic Press, Boston.}
